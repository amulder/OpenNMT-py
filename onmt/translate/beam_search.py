...
class BeamSearch(DecodeStrategy):
    ...
    def advance(self, log_probs, attn):
        ...
        predictions = self.alive_seq
        next_word_indexes = [int(w) for w in self.alive_seq[:,-1]]
        scores_of_next_words = [float(s) for s in self.topk_scores[self._batch_offset[0]]]
        print(
            'DECODER STEP=',step,
            'NUM OF BEAMS=',self.beam_size,
            )
        for beam,(word,score) in enumerate(zip(next_word_indexes,scores_of_next_words)):
            print(
                'BEAM=',beam,
                'NEXT WORD=',word,
                'SCORE=',score
            ...
            if finish_flag and len(self.hypotheses[b]) >= self.n_best:
                ...
                for n, (score, pred, attn) in enumerate(best_hyp):
                    ...
                    print(
                        'HYPOTHESIS_NUM=',n,
                        'SCORE=',float(score),
                        'WORDS_IN_HYP=',pred.shape[0],
                        'HYPOTHESIS=',[int(index) for index in pred]
                        )
            else:
